...
const usePanResponder = ({
      ...
      if (doubleTapToZoomEnabled && isDoubleTapPerformed) {
        ...
        const nextTranslate = isScaled
          ...
          : getTranslateInBounds(
              {
                x: initialTranslate.x + (layout.width / 2 - touchX) * (targetScale / currentScale),
                y: initialTranslate.y + (layout.height / 2 - touchY) * (targetScale / currentScale),
              },
              targetScale,
            );
  ...
  const panResponder = createPanResponder(handlers);
